(function() {
  ...
  //declarations that aren't dependencies.
  var chi; //For a well balanced energy flow
  ...
  function generateMetaData () { //takes inF (in file), outF is a list of all possible values for all properties in inF.
    ...
    voter.find({"District" : {"$eq" : "D"}}, function (err, voterArr) {
      ...
      var file = fs.readFileSync('C:/Users/awimley/pstat/data/metaComb.json', 'utf-8');
  ...
  module.exports.tsvData = function (req, res) {
    //Atempted child process manipulation ot use mongoexport CLI utility in this endpoint.
    //WORKING//var command = "mongoexport --db pstat --collection voters --query \"{'District':{'$eq':'D'},'VotingHistory':{'$size':0}}\" --sort \"{'Residence_WalkListOrder':1}\" --csv -f District,LastVoted,Mail_ZipCode5,Mail_Address1,Mail_Address2,Personal_Age,Personal_FirstName,Personal_LastName,Personal_Race,Personal_Sex,Registration_Date,Registration_PoliticalPartyCode,VotingHistory  -o C:/Users/awimley/pstat/data/data.csv";
    ...
    RegExp.prototype.toJSON = RegExp.prototype.toString; //Regex data types are too rich to be expressed by JSON, so you must simply force a strigify operation when toJSON is called, IE during JSON.stringify()

    //Converting $regex property to literals from a regex string
    var findRegex = Object.keys(q); //Keys through which to search for $regex prop
    for (i in Object.keys(q)) {
      if( q[findRegex[i]].hasOwnProperty("$regex")) {
        //q[findRegex[i]]["$regex"] = RegExp (q[findRegex[i]]["$regex"]); //Case insensitive 'contains' regex
        q[findRegex[i]] = new RegExp (q[findRegex[i]]["$regex"], "i"); 
      }
    }

    var args =[];
    var lolJS= new RegExp("\"", "g");
    var query = JSON.stringify(q);
    query = query.replace(lolJS, "\'"); //Double to single quotes
    query = query.replace(RegExp("\'/", "g"), "/"); //Casting Regex strings to literals.
    query = query.replace(RegExp("/i\'", "g"), "/i");

    var command = "mongoexport --db pstat --collection voters --query \"" +query+ "\" --sort \"{'Residence_WalkListOrder':1}\" --csv -f District,LastVoted,Mail_ZipCode5,Mail_Address1,Mail_Address2,Personal_Age,Personal_FirstName,Personal_LastName,Personal_Race,Personal_Sex,Registration_Date,Registration_PoliticalPartyCode,VotingHistory  -o data/data.csv"
    console.log(command);

    chi = child.exec(command, args);
    ...
    sendJsonResponse(res, 200, "It probably worked.");
  };


  //COMMAND LINE ARGUMENT I NEED TO EXEC (tested, working): 
  /*C:\Users\awimley\pstat>mongoexport --db pstat --collection voters --query "{'District' : {'$eq': 'D'}, 'VotingHistory': {'$size' : 0}}" --sort "{'Residence_WalkListOrder': 1}" --csv -f District,LastVoted,Mail_ZipCode5,Mail_Address1,Mail_Address2,Personal_Age,Personal_FirstName,Personal_LastName,Personal_Race,Personal_Sex,Registration_Date,Registration_PoliticalPartyCode,VotingHistory  -o C:/Users/awimley/pstat/data/data.csv*/
